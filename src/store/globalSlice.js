...
export const login = createAsyncThunk('/login', async (formData) => {
    const queryParams = `filters[$or][0][email][$eq]=${formData.username}&filters[$or][1][username][$eq]=${formData.username}&filters[password][$eq]=${formData.password}&fields[0]=email&fields[1]=aadharNo&fields[2]=username&fields[3]=isPolice`;
        ...
        builder
            ...
            .addCase(login.fulfilled, (state, action) => { // action.payload is the respons
                ...
                if (action.payload.data.length > 0) {
                    ...
                    state.aadharNo = action.payload.data[0].aadharNo;
                    state.isPolice = action.payload.data[0].isPolice;
                    state.message = "Login Successful";

                    localStorage.setItem("profile", JSON.stringify({ ...action.payload.data[0] }));
                } 
